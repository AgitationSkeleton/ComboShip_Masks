...
void func_80835E44(PlayState* play, s16 camSetting) {
    if (!Play_CamIsNotFixed(play)) {
        ...
        Camera_RequestSetting(Play_GetCamera(play, 0), camSetting);
    }
}
...
//   CAM_ID_MAIN/CAM_ID_NONE -> CAM_ID_MAIN/SUBCAM_NONE
//
...
static void Player_InterpolateTransformColors(f32 t, s16* outA, u8* firstA, u8* secondA, u8* biasA, s16* outB,
                                              u8* firstB, u8* secondB, u8* biasB, s16* outC, u8* firstC, u8* secondC,
                                              u8* biasC) {
    s32 i;
...
s32 Player_StartMaskCutscene(PlayState* play, Player* this, s16 csId) {
    ...
    // reported "reveal"). base.type==1 is the engine's own prerender discriminator (Play_CamIsNotFixed in z_play.c,
    ...
    if ((this->subCamId == SUBCAM_NONE) && (csId > 0) && (GET_ACTIVE_CAM(play)->csId != csId) &&
        ...
        this->subCamId = OnePointCutscene_Init(play, csId, 125, &this->actor, CAM_ID_MAIN);
        ...
        // Camera_RequestSetting refuses a priority downgrade while the setting-lock bit (unk_14A & 1, set by the
        ...
        if (this->subCamId != SUBCAM_NONE) {
            ...
            if (subCam != NULL) {
                subCam->unk_14A &= ~1;
                Camera_RequestSetting(subCam, CAM_SET_FREE0);
                subCam->timer = 0x7FFF;
...
void Player_UpdateTransformationAnim(PlayState* play, Player* this) {
    ...
    if (LinkAnimation_Update(play, &this->skelAnime) && (this->skelAnime.animation == (void*)gPlayerAnim_cl_setmask)) {
        ...
        if ((transformFillScreenFlag == 0) && (this->skelAnime.animation == (void*)gPlayerAnim_cl_setmask)) {
            ...
            // FD (2026-07-13): put-on frame 4 is only the SUBTLE mask-attach click (RE D_8085D8F0
            // NA_SE_PL_CHANGE_ARMS). The prominent MM mask sample (0x1850 = our Mask_Attach.wav) is the mask-BREAK at
            // frame 20, NOT here -- firing the full WAV at frame 4 made the transform sound start too early. Keep frame
            // 4 the quiet click.
...
void Player_MaskTransformation(Player* this, PlayState* play) {
    ...
    if (transformFillScreenFlag != 0) {
        if (!sFdMaskHandedOff) {
            ...
            LinkAnimation_Change(play, &this->skelAnime, GET_PLAYER_ANIM(PLAYER_ANIMGROUP_wait, this->modelAnimType),
    ...
    if (this->transformEventTimer1 >= sMaskTransformLightSteps[0].startFrame) {
        if (this->transformEventTimer1 < sMaskTransformLightSteps[0].midFrame) {
            ...
            if (this->transformEventTimer1 == sMaskTransformLightSteps[0].midFrame) {
                Sfx_PlaySfxCentered(
                    NA_SE_EV_LIGHTNING); // FD (2026-07-11): func_800788CC->Sfx_PlaySfxCentered, no _HARD
            }
            ...
            if ((play->bombchuBowlingStatus == 0) &&
                ...
                if ((play->ageChangeFlag < 0) && !(this->stateFlags3 & PLAYER_STATE3_TRANSFORMATION_MASK) &&
                    ...
                    if (LINK_IS_DEITY) {
                        ...
                        if (nextForm > LINK_AGE_CHILD) {
                            nextForm = LINK_AGE_ADULT;
                        } // safety clamp
                    } else {
...
void func_80838F5C(PlayState* play, Player* this) {
    ...
    Camera_RequestSetting(Play_GetCamera(play, 0), CAM_SET_FREE0);
}
        ...
        if ((CHECK_BTN_ALL(sControlInput->press.button, BTN_A) || (Player_Action_8084F9A0 == this->actionFunc)) &&
            ...
            if (this->doorType == PLAYER_DOORTYPE_SLIDING) {
                ...
                door->animStyle = (doorDirection < 0.0f)
                                      ? ((LINK_IS_ADULT || fdDoorAsAdult) ? KNOB_ANIM_ADULT_L : KNOB_ANIM_CHILD_L)
...
s32 func_8083AD4C(PlayState* play, Player* this) {
    ...
    if (this->unk_6AD == 2) {
        if (func_8002DD6C(this)) {
            ...
            camMode = shouldUseBowCamera ? CAM_MODE_AIM_ADULT : CAM_MODE_AIM_CHILD;
        } else {
            // #region SOH [Enhancement]
    ...
    if (iREG(67) ||
        ...
        if (iREG(67) || (this->getItemId > GI_NONE)) {
            ...
            if (giEntry.collectable) {
                ...
                uint8_t showItemCutscene =
                    play->sceneNum == SCENE_BOMBCHU_BOWLING_ALLEY || IS_RANDO || giEntry.modIndex == MOD_RANDOMIZER ||
                    ...
                    giEntry.itemId == ITEM_MASK_DEITY || Item_CheckObtainability(giEntry.itemId) == ITEM_NONE;
...
void Player_Init(Actor* thisx, PlayState* play2) {
    ...
    thisx->room = -1;
    this->ageProperties = &sAgeProperties[gSaveContext.linkAge];
    this->transformTargetForm =
        gSaveContext.linkAge; // FD (2026-07-11) 4d (light/maskObjectSegment = cutscene only, skipped)
...
void Player_UpdateCommon(Player* this, PlayState* play, Input* input) {
    ...
    // natural completion OR an A-button skip (the skip just transitions the action away from
    // Player_MaskTransformation). By a natural end the scream has already finished, so this only actually cuts sound on
    // a skip.
    {
    ...
    if (this->actor.category == ACTORCAT_PLAYER) {
        ...
        if (this->naviWarning != 0) {
            ...
            if (LINK_IS_DEITY && CVarGetInteger(CVAR_ENHANCEMENT("TransformationMasks.StuckSafeguards"), 1)) {
                ...
                s32 hasWaterProblems = ((play->sceneNum == SCENE_ZORAS_RIVER) && (play->roomCtx.curRoom.num == 1)) ||
...
s32 func_8084DFF4(PlayState* play, Player* this) {
    ...
    if (this->av1.actionVar1 == 0) {
        ...
        if (giEntry.modIndex == MOD_NONE) {
            ...
            // FD (P4 TODO): a rando-obtained Fierce Deity's Mask should play the custom "Get a Mask" fanfare
            // (FdAudio_PlayOneShot("custom/samples/fd/Get_A_Mask.wav")); the ComboShip rando FD-mask get-enum
            // wiring is deferred to P4 (shared FD-mask rando), so for now it falls through to the rando fanfare.
            if (IS_RANDO) {
                Audio_PlayFanfare_Rando(giEntry);
                ...
                this->stateFlags2 |= PLAYER_STATE2_SPIN_ATTACKING;
                {
                    s16 beamPitch =
                        (this->focusActor != NULL)
                            ? Math_Vec3f_Pitch(&this->bodyPartsPos[PLAYER_BODYPART_WAIST], &this->focusActor->focus.pos)
                            : 0;
