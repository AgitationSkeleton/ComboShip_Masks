...
#include "macros.h"
#include "variables.h"
#include "functions.h" // FD (aegiker RE->SoH port): Player_GetFdTransformAnimId (Anchor transform-mask networking)
extern PlayState* gPlayState;
// FD (aegiker RE->SoH port): the FD mask's get-item uses this custom draw func (its gid is the Goron-mask flow-gate
// id, so remotes can't tell it apart by gid alone); we sync a bit when the local player is holding it up.
...
void Anchor::SendPacket_PlayerUpdate() {
    ...
    // FD (aegiker RE->SoH port): Fierce Deity transform-cutscene state so remotes see the mask donning/scream + morph.
    ...
    // FD (aegiker RE->SoH port): true while this player is holding up the FD mask get-item, so remotes draw the FD
    // mask model (via its custom draw func) instead of the raw Goron mask its gid maps to.
...
void Anchor::HandlePacket_PlayerUpdate(nlohmann::json payload) {
    ...
    if (clients.contains(clientId)) {
        ...
        // FD (aegiker RE->SoH port): Fierce Deity transform-cutscene state (defaults keep pre-FD clients / non-FD
        // frames inert).
