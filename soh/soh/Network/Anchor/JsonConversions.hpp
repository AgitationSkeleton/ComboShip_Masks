...
inline void to_json(json& j, const ShipSaveContextData& shipSaveContextData) {
    j = json{
        ...
        // FD (aegiker RE->SoH port): the Fierce Deity mask is a custom ownership bool (no inventory slot / item-table
        // entry), so it was previously carried by nothing. Ride it along here so a late-joining / reconnecting client
        // reconciles it via UPDATE_TEAM_STATE (applied OR-merged in HandlePacket_UpdateTeamState).
...
inline void from_json(const json& j, ShipSaveContextData& shipSaveContextData) {
    ...
    // FD (aegiker RE->SoH port): default-tolerant (.value) so a client on an older build that omits the key doesn't
