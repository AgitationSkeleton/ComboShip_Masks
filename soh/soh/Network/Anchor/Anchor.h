...
typedef struct {
    ...
    // FD (aegiker RE->SoH port): Fierce Deity transform-cutscene state, so remote players SEE another player donning
    // the mask (held mask -> on-face mask -> scream) and morphing into / out of Fierce Deity. The static FD form
    // already replicates via linkAge/modelGroup/jointTable; these drive the extra mask draws in
