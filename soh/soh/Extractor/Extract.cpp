...
std::string Extractor::ExtractCuratedToTemp(std::string installPath, std::atomic<size_t>* extractCount,
    ...
    // ComboShip: FD generation uses a DEDICATED minimal curated MM XML subset (assets/fd_xml/<ver>, 4 objects:
    // gameplay_keep + object_link_boy[FD body] + object_mask_boy + object_gi_mask03) rather than the full MM
    // extraction set at assets/xml/<ver> — keeps ComboShip's normal MM extraction untouched and makes fd.o2r
    // generation fast (FD-only, no full MM re-extract). Set below (xmlPath).
    ...
    snprintf(xmlPath, 1024, "assets/fd_xml/%s", version);
