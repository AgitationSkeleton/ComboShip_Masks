...
bool FdO2rGen::Generate(const std::string& installPath, const std::string& dataPath, const std::string& appShortName,
    ...
    // here. (soh_fd's standalone build used autoMode as a headless "generate fd.o2r and exit" CI pass; ComboShip
    // has no separate headless pass, so that std::exit(0) was removed.)
    (void)autoMode;
    return true;
}
